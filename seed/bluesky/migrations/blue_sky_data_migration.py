...
def get_cycle(apps, year_ending, user):
    ...
    obj, created = Cycle.objects.get_or_create(user = user, name = name, start = year_start, end = year_end)
    obj.save()
    
    return obj

def create_property_record(apps, campus, parent_property):
    PropertyModel = apps.get_model("seed", "property")
    rec = PropertyModel.objects.create(campus = campus, parent_property = parent_property)
    rec.save()
    return rec

def create_taxlot_record(apps):
    TaxLot = apps.get_model("seed", "TaxLot")
    rec = TaxLot.objects.create()
    rec.save()
    return rec
...
def process_record(apps, bs_record, buildingsnapshot_to_propertystate_mappings, propertystate_fields_to_check_for_in_extra_data, buildingsnapshot_to_taxlot_mappings, taxlot_fields_to_check_for_in_extra_data):
    ...
    SeedUser = apps.get_model("landing", "seeduser")
    user = SeedUser.objects.get(id=1)
    ...
    cycle = get_cycle(apps, bs_record.year_ending, user)
    
    #just default the organization, campus, and parent_property fields for now
    property_record = create_property_record(apps, campus = False, parent_property = None) 
    
    #just default the organization for now
    taxlot_record = create_taxlot_record(apps)
