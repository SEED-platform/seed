...
def get_cycle(apps, year_ending, org):
    ...
    Cycle = apps.get_model("bluesky", "Cycle")
...
def create_property_record(apps, org, campus, parent_property):
    PropertyModel = apps.get_model("bluesky", "property")
...
def create_taxlot_view_record(apps, taxlot, cycle, taxlot_state):
    TaxLotView = apps.get_model("bluesky", "TaxLotView")
...
def create_taxlot_property_record(apps, cycle, property_view, taxlot_view, primary):
    TaxLotProperty = apps.get_model("bluesky", "TaxLotProperty")
...
def get_property_state_to_building_snapshot_field_mapping(apps):
    PropertyState = apps.get_model("bluesky", "PropertyState")
...
def get_taxlot_state_to_building_snapshot_field_mapping(apps):
    TaxLotState = apps.get_model("bluesky", "TaxLotState")
    BuildingSnapshot = apps.get_model("seed", "BuildingSnapshot")
...
def copy_old_world_to_blue_sky(apps):
    BuildingSnapshot = apps.get_model("seed", "BuildingSnapshot")
...
class Command(BaseCommand):
    ...
    help = 'Copies over data from the existing building tables '

    def handle(self, *args, **options):
        print "Running initialize Blue sky Data."

        canonical_building_cnt = apps.get_model("seed", "CanonicalBuilding").objects.count()
        cycle_cnt = apps.get_model("bluesky", "Cycle").objects.count()

        print "There are {} Canonical Buildings".format(canonical_building_cnt)
        print "There are {} Cycles".format(cycle_cnt)

        # copy_old_world_to_blue_sky(apps)
