...
def remove_duplicate_users_for_org(apps, _schema_editor):
    ...
    duplicate_groups = OrganizationUser.objects.values(*unique_fields).order_by().annotate(count=Count('user_id')).filter(count__gt=1)

    for duplicate_group in duplicate_groups:
        duplicates = OrganizationUser.objects.filter(**{x: duplicate_group[x] for x in unique_fields}).order_by('-role_level', 'id')
