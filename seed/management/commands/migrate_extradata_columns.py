...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--no-update-columns', dest='update_columns', default=True, action="store_false")
        parser.add_argument(
            '--update-columns', dest='update_columns', default=True, action="store_true")

        parser.add_argument('--no-add-unmapped-columns',
                            dest='add_unmapped_columns', default=True, action="store_false")
        parser.add_argument('--add-unmapped-columns',
                            dest='add_unmapped_columns', default=True, action="store_true")

        parser.add_argument('--no-create-missing-columns',
                            dest='create_missing_columns', default=True, action="store_false")
        parser.add_argument('--create-missing-columns',
                            dest='create_missing_columns', default=True, action="store_true")
        return
    ...
    def find_missing_columns(self, org, create):
        ...
        property_views = PropertyView.objects.filter(
            property__organization=org).select_related('state').all()
        taxlot_views = TaxLotView.objects.filter(
            taxlot__organization=org).select_related('state').all()

        property_keys = set(itertools.chain.from_iterable(
            map(lambda v: v.state.extra_data.keys(), property_views)))
        taxlot_keys = set(itertools.chain.from_iterable(
            map(lambda v: v.state.extra_data.keys(), taxlot_views)))
        ...
        for pk in property_keys:
            ...
            if not cnt:
                if create:
                    logging_info(
                        "Creating missing column {} found in property extra_data keys".format(pk))
                    col = Column(organization=org, column_name=pk,
                                 is_extra_data=True, extra_data_source=Column.SOURCE_PROPERTY)
                    col.save()
                else:
                    logging_info(
                        "Missing column {} found in property extra_data keys".format(pk))
        ...
        for tlk in taxlot_keys:
            ...
            if not cnt:
                if create:
                    logging_info(
                        "Creating missing column {} found in tax lot extra data keys".format(tlk))
                    col = Column(organization=org, column_name=tlk,
                                 is_extra_data=True, extra_data_source=Column.SOURCE_PROPERTY)
                    col.save()
                else:
                    logging_info(
                        "Missing column {} found in tax lot extra data keys.".format(tlk))
    ...
    def update_columns(self, org, update_columnms=True, create_missing=False):
        ...
        for tl_col in taxlot_columns:
            ...
            if cnt:
                ...
                col = Column(organization=org, column_name=tl_col,
                             is_extra_data=True, extra_data_source=Column.SOURCE_TAXLOT)
        ...
        for prop_col in property_columns:
            ...
            if cnt:
                ...
                # Create the missing column
                col = Column(organization=org, column_name=prop_col,
                             is_extra_data=True, extra_data_source=Column.SOURCE_PROPERTY)
                logging_info("CREATING COLUMN {}".format(col))
                col.save()
        return
