...
if_name = os.path.expanduser("~/Source/LBL/tmp/tmp.csv")
with open(if_name) as tree_file:
...
def makeProj(n):
    def proj(vect): return vect[n]
    return proj

all_nodes = set(map(makeProj(0), m2m)).union(set(map(makeProj(1), m2m)))
...
nontrivial_tree_labels = set(map(makeProj(0), filter(lambda (x,y): y > 1, counts.items())))
