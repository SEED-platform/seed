...
from seed.test_helpers.fake import FakeAnalysisFactory, FakeCycleFactory, FakeNoteFactory, FakePropertyFactory
...
class EventTests(TransactionTestCase):
    ...
    def test_get_all_notes(self):
        # Setup
        at_event = ATEvent.objects.create(property=self.property, cycle=self.cycle, building_file=BuildingFile.objects.create())
        at_event.save()

        analysis_event = AnalysisEvent.objects.create(property=self.property, cycle=self.cycle, analysis=self.analysis)
        analysis_event.save()

        note_event = NoteEvent.objects.create(property=self.property, cycle=self.cycle, note=self.note)
        note_event.save()

        # wrong property, should not be included
        analysis_event = AnalysisEvent.objects.create(
            property=self.property_factory.get_property(), cycle=self.cycle, analysis=self.analysis
        )
        analysis_event.save()
