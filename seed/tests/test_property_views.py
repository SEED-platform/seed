...
class PropertyMeterViewTests(DataMappingBaseTestCase):
    ...
    def test_property_meter_usage_can_filter_when_usages_of_a_partial_month(self):
        save_raw_data(self.import_file.id)
        ...
        # 2020 January partial month
        reading_details = {
            'meter_id': meter.id,
            'start_time': make_aware(datetime(2020, 1, 10, 0, 0, 0), timezone=tz_obj),
            'end_time': make_aware(datetime(2020, 1, 20, 0, 0, 0), timezone=tz_obj),
            'reading': 100 * 3.41,
            'source_unit': 'kBtu (thousand Btu)',
            'conversion_factor': 1
        }
