...
from seed.utils.geocode import MapQuestAPIKeyError, bounding_box_wkt, geocode_buildings, long_lat_wkt
...
class GeocodeAddresses(TestCase):
    ...
    def test_geocode_properties_with_custom_fields(self):
        with base_vcr.use_cassette('seed/tests/data/vcr_cassettes/geocode_property_custom_fields.yaml'):
            ...
            self.org.column_set.filter(column_name='pm_parent_property_id', table_name='PropertyState').update(geocoding_order=1)
            self.org.column_set.filter(column_name='pm_property_id', table_name='PropertyState').update(geocoding_order=2)
            self.org.column_set.create(column_name='ed_city', is_extra_data=True, table_name='PropertyState', geocoding_order=3)
            self.org.column_set.filter(column_name='state', table_name='PropertyState').update(geocoding_order=4)
            self.org.column_set.create(column_name='ed_zip', is_extra_data=True, table_name='PropertyState', geocoding_order=5)
            self.org.column_set.create(column_name='ed_empty', is_extra_data=True, table_name='PropertyState', geocoding_order=6)
            self.org.column_set.filter(column_name='property_name', table_name='PropertyState').update(geocoding_order=7)

            # Deactivate default geocoding columns
            self.org.column_set.filter(
                column_name__in=['address_line_1', 'address_line_2', 'city', 'postal_code'], table_name='PropertyState'
    ...
    def test_geocode_taxlots_with_custom_fields(self):
        with base_vcr.use_cassette('seed/tests/data/vcr_cassettes/geocode_taxlots_custom_fields.yaml'):
            ...
            # Activate and order geocoding columns
            self.org.column_set.filter(column_name='jurisdiction_tax_lot_id', table_name='TaxLotState').update(geocoding_order=1)
            self.org.column_set.filter(column_name='block_number', table_name='TaxLotState').update(geocoding_order=2)
            self.org.column_set.create(column_name='ed_city', is_extra_data=True, table_name='TaxLotState', geocoding_order=3)
            self.org.column_set.filter(column_name='state', table_name='TaxLotState').update(geocoding_order=4)
            self.org.column_set.create(column_name='ed_zip', is_extra_data=True, table_name='TaxLotState', geocoding_order=5)
            self.org.column_set.create(column_name='ed_empty', is_extra_data=True, table_name='TaxLotState', geocoding_order=6)
            self.org.column_set.filter(column_name='custom_id_1', table_name='TaxLotState').update(geocoding_order=7)

            # Deactivate default geocoding columns
            self.org.column_set.filter(
                column_name__in=['address_line_1', 'address_line_2', 'city', 'postal_code'], table_name='TaxLotState'
            ).update(geocoding_order=0)
    ...
    def test_not_enough_geocoding_fields_for_org_leads_to_no_geocoding(self):
        ...
        self.org.column_set.filter(table_name='PropertyState').update(geocoding_order=0)
    ...
    def test_geocode_buildings_is_successful_with_over_100_properties(self):
        with batch_vcr.use_cassette('seed/tests/data/vcr_cassettes/geocode_101_unique_addresses.yaml', match_on=['uri_length']):
            ...
            long_lats = [property.long_lat for property in refreshed_properties if property.long_lat is not None]
            geocode_confidence_results = [
                property.geocoding_confidence for property in refreshed_properties if property.geocoding_confidence is not None
            ]
    ...
    def test_geocode_buildings_is_unsuccessful_when_the_API_key_is_invalid_or_expired(self):
        with base_vcr.use_cassette('seed/tests/data/vcr_cassettes/geocode_invalid_or_expired_key.yaml'):
            ...
            with pytest.raises(MapQuestAPIKeyError):
                geocode_buildings(properties)
