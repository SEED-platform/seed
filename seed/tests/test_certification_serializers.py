...
from seed.test_helpers.fake import FakeGreenAssessmentFactory, FakeGreenAssessmentPropertyFactory, FakePropertyViewFactory
...
class TestFields(DeleteModelsTestCase):
    def get_date_string(self, dtobj):
        """Get YY-MM-DD string"""
        return f"{dtobj.year}-{dtobj.month}-{dtobj.day}"
    ...
    def test_url_field(self):
        ...
        expected = [("http://example.com", "example.com"), ("http://example.org", "example.org")]
    ...
    def test_validity_duration_field_to_representation(self):
        ...
        green_assessment = ga_factory.get_green_assessment(validity_duration=365)
...
class TestGreenAssessmentPropertySerializer(DeleteModelsTestCase):
    ...
    def test_create(self, mock_url_model):
        ...
        mock_url_model.objects.bulk_create.assert_called_with(
            [mock_url_model(property_assessment=instance, url=url[0], description=url[1]) for url in self.urls]
        )
    ...
    def test_update(self, mock_url_model):
        ...
        mock_url_model.objects.filter.assert_called_with(property_assessment=instance)
        mock_url_model.assert_called_with(url="http://example.org", property_assessment=gap)
...
class TestGreenAssessmentSerializer(DeleteModelsTestCase):
    def setUp(self):
        ...
        self.assessment = self.ga_factory.get_green_assessment(**assessment_data)
