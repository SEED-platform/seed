...
class TestPropertyMeasures(DeleteModelsTestCase):
    ...
    def test_get_property_measure(self):
        ...
        property_measure0 = PropertyMeasure.objects.create(
            measure=measures[0], property_state=property_state, description="Property Measure 0"
        )
        property_measure1 = PropertyMeasure.objects.create(
            measure=measures[1], property_state=property_state, description="Property Measure 1"
        )
        property_measure2 = PropertyMeasure.objects.create(
            measure=measures[2], property_state=property_state, description="Property Measure 2"
        )
        property_measure3 = PropertyMeasure.objects.create(
            measure=measures[3], property_state=property_state, description="Property Measure 3"
        )
        ...
        url = reverse_lazy("api:v3:property-measures-list", args=[property_view.id, 1234567]) + f"?organization_id={self.org.id}"
        ...
        url = reverse_lazy("api:v3:property-measures-list", args=[property_view.id, scenario0.id]) + f"?organization_id={self.org.id}"
        response = self.client.get(url, **self.headers)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["status"], "success")
        data = response.json()["data"]
        ...
        url = reverse_lazy("api:v3:property-measures-list", args=[property_view.id, scenario1.id]) + f"?organization_id={self.org.id}"
        response = self.client.get(url, **self.headers)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["status"], "success")
        data = response.json()["data"]
        ...
        url = (
            reverse_lazy("api:v3:property-measures-detail", args=[property_view.id, scenario1.id, property_measure2.id])
        ...
        url = (
            reverse_lazy("api:v3:property-measures-detail", args=[property_view.id, scenario1.id, 1234]) + f"?organization_id={self.org.id}"
        )
        response = self.client.get(url, **self.headers)
    ...
    def test_update_property_measure(self):
        ...
        scenario0 = Scenario.objects.create(property_state=property_state, name="scenario 0")
        property_measure0 = PropertyMeasure.objects.create(
            measure=measures[0], property_state=property_state, description="Property Measure 0", implementation_status=1
        )
        property_measure1 = PropertyMeasure.objects.create(
            measure=measures[1], property_state=property_state, description="Property Measure 1", implementation_status=1
        )
        property_measure0.scenario_set.add(scenario0.id)
        property_measure1.scenario_set.add(scenario0.id)

        property_measure_fields = {"description": "updated desc", "implementation_status": 7}

        url = (
            reverse_lazy("api:v3:property-measures-detail", args=[property_view.id, scenario0.id, property_measure1.id])
            + f"?organization_id={self.org.id}"
        )

        response = self.client.put(url, data=json.dumps(property_measure_fields), content_type="application/json", **self.headers)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["status"], "success")
        data = response.json()["data"]
    ...
    def test_fail_to_update_property_measure_with_invalid_data(self):
        ...
        property_measure0.scenario_set.add(scenario0.id)
        # Invalid Field Name
        property_measure_fields = {"description": "updated desc", "invalid_field": 123}

        url = (
            reverse_lazy("api:v3:property-measures-detail", args=[property_view.id, scenario0.id, property_measure0.id])
            + f"?organization_id={self.org.id}"
        )
        ...
        url = (
            reverse_lazy("api:v3:property-measures-detail", args=[property_view.id, scenario0.id, 99999])
            + f"?organization_id={self.org.id}"
        )
        ...
        url = (
            reverse_lazy("api:v3:property-measures-detail", args=[9999, scenario0.id, property_measure0.id])
            + f"?organization_id={self.org.id}"
        )
    ...
    def test_delete_property_measure(self):
        ...
        response = self.client.delete(
            reverse_lazy("api:v3:property-measures-detail", args=[property_view.id, scenario.id, 9999]) + f"?organization_id={self.org.id}",
            **self.headers,
        )
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.json()["status"], "error")
...
class TestPropertyMeasuresPermissions(AccessLevelBaseTestCase):
    def setUp(self):
        ...
        self.property_measure = PropertyMeasure.objects.create(
            measure=self.measures[5], property_state=self.property_state, description="Property Measure"
        )
        self.property_measure.scenario_set.add(self.scenario.id)

    def test_measures_list(self):
        url = (
            reverse_lazy("api:v3:property-measures-list", args=[self.property_view.id, self.scenario.id])
            + f"?organization_id={self.org.id}"
        )
    ...
    def test_measures_get(self):
        url = (
            reverse_lazy("api:v3:property-measures-detail", args=[self.property_view.id, self.scenario.id, self.property_measure.id])
            + f"?organization_id={self.org.id}"
        )
