...
class AuditTemplateViewTests(TestCase):
    ...
    def test_get_building_xml_from_audit_template_org_has_no_at_token(self, mock_request):
        ...
        self.assertEqual(
            response.json(), {'message': 'An Audit Template organization token, user email and password are required!', 'success': False}
        )
    ...
    def test_get_building_xml_from_audit_template_bad_at_authentication_response(self, mock_request):
        ...
        self.assertEqual(
            ...
            {
                'success': False,
                'message': f'Expected 200 response from Audit Template get_api_token but got 400: {self.bad_authenticate_response.content}',
            },
        )
    ...
    def test_get_building_xml_from_audit_template_bad_at_get_building_response(self, mock_request):
        ...
        self.assertEqual(
            ...
            {
                'success': False,
                'message': f'Expected 200 response from Audit Template get_building_xml but got 400: {self.bad_get_building_response.content}',
            },
        )
...
class AuditTemplateBatchTests(TestCase):
    ...
    def test_get_buildings_from_audit_template_bad_authentication(self, mock_request):
        ...
        exp_json = {
            'success': False,
            'message': "Expected 200 response from Audit Template get_api_token but got 400: {'error': 'Invalid email, password or organization_token.'}",
        }
    ...
    def test_batch_get_building_xml(self, mock_request):
        ...
        url = (
            reverse('api:v3:audit_template-batch-get-building-xml')
            + '?organization_id='
            + str(self.org.id)
            + '&cycle_id='
            + str(self.cycle.id)
            ...
            [
                {
                    ...
                    'email': 'test@test.com',
                    'name': 'name1',
                    'updated_at': '2020-01-01T01:00:00.000000',
                },
                {
                    'audit_template_building_id': 2,
                    'property_view': self.view2.id,
