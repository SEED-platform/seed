...
class PropertyStateSerializer(serializers.ModelSerializer):
    ...
    site_wui = PintQuantitySerializerField(allow_null=True)
...
class PropertyStatePromoteWritableSerializer(serializers.ModelSerializer):
    ...
    site_wui = PintQuantitySerializerField(allow_null=True, required=False)
...
def unflatten_values(vdict, fkeys):
    ...
    :type fkeys: list
    """
    if not set(vdict.keys()).isdisjoint(set(fkeys)):
        raise ValueError(f"unflatten_values: {vdict} has fields named in {fkeys}")
