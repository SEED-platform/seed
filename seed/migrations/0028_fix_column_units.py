...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Updates organiaztion columns that were derived from global column
           definitions but failed to associate the global unit with the newly
           created organization's column."""
        # Note: Don't use "from appname.models import ModelName".
        # Use orm.ModelName to refer to models in this application,
        # and orm['appname.ModelName'] for models in other applications.
        orgs = orm.Column.objects.all()
        for org in orgs:
            org_cols = orm.Column.objects.filter(
                organization=org,
                unit__isnull=True,
            )

            for col in org_cols:
                global_col = orm.Column.objects.filter(
                    column_name=col.column_name,
                    organization=None
                ).first()

                if global_col and global_col.unit:
                    col.unit = global_col.unit
                    col.save()

    def backwards(self, orm):
        """
        We lose information in the forwards migration and don't want to
        make any poor assumptions and destroy data in the backwards one.
        It should be ok since the forwards migration is intended to fix
        data that was imported incorrectly in the first play due to buggy
        mapping code.
        """
        pass
