...
def get_filtered_results(request: Request, inventory_type: Literal["property", "taxlot"], profile_id: int) -> JsonResponse:
    ...
    if include_related:
        ...
        try:
            filters, annotations, _ = build_view_filters_and_sorts(
                request.query_params, other_columns_from_database, other_inventory_type, include_sorts=False
            )
        except FilterError as e:
            return JsonResponse({"status": "error", "message": f"Error filtering: {e!s}"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if len(filters) > 0 or len(annotations) > 0:
            other_inventory_type_class: type[TaxLotView] | type[PropertyView] = TaxLotView if inventory_type == "property" else PropertyView
