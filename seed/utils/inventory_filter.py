...
def get_filtered_results(request: Request, inventory_type: Literal["property", "taxlot"], profile_id: int) -> JsonResponse:
    ...
    goal_related_model_sort = any(substring in order_by_param for substring in ["property__goal_note", "property__historical_note"])
    if goal_related_model_sort:
        build_filter_and_sort_fxn = build_related_model_filters_and_sorts
    else:
        build_filter_and_sort_fxn = build_view_filters_and_sorts

    try:
        filters, annotations, order_by = build_filter_and_sort_fxn(
