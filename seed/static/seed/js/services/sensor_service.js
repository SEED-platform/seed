...
angular.module('BE.seed.service.sensor', []).factory('sensor_service', [
  ...
  function ($http) {
    ...
    sensor_factory.get_data_loggers = function (property_view_id, organization_id) {
      return $http.get('/api/v3/data_loggers/', { params: { property_view_id, organization_id } }).then(function (response) {
    ...
    sensor_factory.get_sensors = function (property_view_id, organization_id) {
      return $http.get('/api/v3/properties/' + property_view_id + '/sensors/', { params: { organization_id } }).then(function (response) {
        return response.data;
      });
    };
