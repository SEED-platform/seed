...
angular.module('BE.seed.service.meter', []).factory('meter_service', [
  ...
  function ($http) {
    ...
    meter_factory.get_meters = function (property_view_id, organization_id) {
      return $http.get('/api/v3/properties/' + property_view_id + '/meters/', { params: { organization_id } }).then(function (response) {
        return response.data;
      });
    };

    meter_factory.delete_meter = function (property_view_id, meter_id) {
      return $http.delete('/api/v3/properties/' + property_view_id + '/meters/' + meter_id).then(function (response) {
        return response.data;
      });
    };
