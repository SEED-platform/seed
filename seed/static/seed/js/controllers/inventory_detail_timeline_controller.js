...
angular.module('BE.seed.controller.inventory_detail_timeline', []).controller('inventory_detail_timeline_controller', [
  ...
  function ($scope, $stateParams, $timeout, uiGridConstants, cycles, events, inventory_payload, urls, users_payload, organization_payload) {
    ...
    const formatTimeline = (events) => {
      ...
      eventsByCycle.sort((a, b) => ($scope.orderDesc ? new Date(a.cycle_end_date) - new Date(b.cycle_end_date) : new Date(b.cycle_end_date) - new Date(a.cycle_end_date)));
    ...
    $scope.eventSelectGridOptions = {
      ...
      columnDefs: [{ field: 'eventType', visible: false }, { field: 'Select All' }],
      enableColumnMenus: false,
