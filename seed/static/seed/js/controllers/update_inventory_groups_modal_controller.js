angular.module('SEED.controller.update_inventory_groups_modal', [])
  .controller('update_inventory_groups_modal_controller', [
    ...
    function ($scope, $log, $uibModalInstance, inventory_group_service, inventory_ids, inventory_type, org_id, notification) {
      ...
      $scope.initialize_new_group = function () {
        $scope.new_group = {
      ...
      $scope.toggle_add = function (group) {
      ...
      $scope.toggle_remove = function (group) {
        if (group.is_checked_remove && group.is_checked_add) {
      ...
      $scope.modified = function () {
        return Boolean(_.filter($scope.inventory_groups, 'is_checked_add').length || _.filter($scope.inventory_groups, 'is_checked_remove').length);
      };

      /* User has indicated 'Done' so perform selected group operations */
      $scope.done = function () {
        var addGroupIDs = _.chain($scope.inventory_groups).filter('is_checked_add').map('id').value().sort();
        var removeGroupIDs = _.chain($scope.inventory_groups).filter('is_checked_remove').map('id').value().sort();

        if (inventory_type === 'properties') {
          inventory_group_service.update_inventory_groups(addGroupIDs, removeGroupIDs, inventory_ids, 'property').then(function (data) {
          ...
          inventory_group_service.update_inventory_groups(addGroupIDs, removeGroupIDs, inventory_ids, 'tax_lot').then(function (data) {
            if (data.num_updated === 1) {
      ...
      var init = function () {
        $scope.initialize_new_group();
        $scope.loading = true;
        inventory_group_service.get_groups_for_inventory(inventory_type, inventory_ids).then(function (groups) {
          $scope.inventory_groups = [];
