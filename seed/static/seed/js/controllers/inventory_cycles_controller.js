 ...
 */
angular.module('SEED.controller.inventory_cycles', []).controller('inventory_cycles_controller', [
  '$scope',
  '$window',
  '$stateParams',
  'inventory_service',
  ...
  function (
    ...
    const state_type = $scope.inventory_type === 'properties' ? 'PropertyState' : 'TaxLotState';
    ...
    $scope.refresh_objects = () => {
      ...
      if ($scope.inventory_type === 'properties') {
        ...
        inventory_service.taxlots_cycle(profile_id, $scope.included_cycle_ids).then((refreshed_inventory) => {
          $scope.data = $scope.format_records(refreshed_inventory);
          spinner_utility.hide();
        });
      }
    ...
    $scope.build_columns = () => {
      ...
      if ($scope.currentProfile) {
        _.forEach($scope.currentProfile.columns, (col) => {
          const foundCol = _.find($scope.all_columns, { id: col.id });
          if (foundCol) {
