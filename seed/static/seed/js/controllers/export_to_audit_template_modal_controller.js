 ...
 */
angular.module('BE.seed.controller.export_to_audit_template_modal', [])
    .controller('export_to_audit_template_modal_controller', [
        ...
        (
            ...
            $scope.ids = {'property_view_ids': ids};
            $scope.org_id = org_id
            $scope.step = {number: 0}
            $scope.uploader = {
            ...
            $scope.export = () => {
                audit_template_service.batch_export_to_audit_template($scope.org_id, $scope.ids).then(response => {
                    $scope.step.number = 1
