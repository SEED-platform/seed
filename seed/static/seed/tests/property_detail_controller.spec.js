...
describe('controller: property_detail_controller', function () {
  ...
  var property_detail_controller, property_detail_controller_scope;
  ...
  beforeEach(inject(
    function ($controller,
      ...
      property_detail_controller_scope = $rootScope.$new();
      modal_state = '';
      delete_called = false;
      ...
      spyOn(mock_inventory_service, 'update_property')
        .andCallFake(function (property_id, cycle_id, property_state) {
            property_detail_controller_scope.item_state = property_state;
            return $q.when({
              status: 'success'
            });
          }
  ...
  function create_property_detail_controller() {
    ...
    property_detail_controller = controller('property_detail_controller', {
      $controller: controller,
      $scope: property_detail_controller_scope,
  ...
  it('should have a Property payload with correct object properties', function () {
    ...
    // act
    property_detail_controller_scope.$digest();

    // assertions
    expect(property_detail_controller_scope.property.id).toBe(4);
    expect(property_detail_controller_scope.cycle.id).toBe(1);
    expect(property_detail_controller_scope.item_state.address_line_1).toBe("123 Main St.");
  ...
  it('should make a copy of Property while making edits', function () {
    ...
    // act
    property_detail_controller_scope.$digest();
    property_detail_controller_scope.on_edit();
    property_detail_controller_scope.item_state.address_line_1 = "ABC Main St.";

    // assertions
    expect(property_detail_controller_scope.item_copy.address_line_1).toBe("123 Main St.");
  ...
  it('should restore enabled the edit fields if a user clicks edit', function () {
    ...
    // assertions
    expect(property_detail_controller_scope.edit_form_showing).toBe(true);
  ...
  it('should restore the copy of Property state if a user clicks cancel', function () {
    ...
    property_detail_controller_scope.on_edit();
    property_detail_controller_scope.item_state.address_line_1 = "ABC Main St.";
    property_detail_controller_scope.on_cancel();

    // assertions
    expect(property_detail_controller_scope.item_state.address_line_1).toBe("123 Main St.");
    expect(property_detail_controller_scope.edit_form_showing).toBe(false);
  ...
  it('should save the Property state when a user clicks the save button', function () {
    ...
    property_detail_controller_scope.on_edit();
    property_detail_controller_scope.item_state.address_line_1 = "ABC Main St.";
    property_detail_controller_scope.on_save();

    // assertions
    expect(mock_inventory_service.update_property)
      .toHaveBeenCalledWith(property_detail_controller_scope.property.id,
        property_detail_controller_scope.cycle.id,
        property_detail_controller_scope.item_state);
    expect(property_detail_controller_scope.item_state.address_line_1).toEqual("ABC Main St.");
  });
  ...
  it('should hide certain Property properties, including ids and extra_data', function () {
    ...
    // assertions
    expect(property_detail_controller_scope.is_valid_data_column_key('id')).toEqual(false);
    expect(property_detail_controller_scope.is_valid_data_column_key('pk')).toEqual(false);
    expect(property_detail_controller_scope.is_valid_data_column_key('pk_source')).toEqual(false);
    expect(property_detail_controller_scope.is_valid_data_column_key('extra_data ')).toEqual(false);
    expect(property_detail_controller_scope.is_valid_data_column_key('address_line_1')).toEqual(true);
