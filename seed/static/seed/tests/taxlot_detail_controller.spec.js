...
describe('controller: taxlot_detail_controller', function () {
  ...
  var taxlot_detail_controller, taxlot_detail_controller_scope;
  ...
  beforeEach(inject(
    function ($controller,
      ...
      taxlot_detail_controller_scope = $rootScope.$new();
      modal_state = '';
      delete_called = false;
      ...
      spyOn(mock_inventory_service, 'update_taxlot')
        .andCallFake(function (taxlot_id, cycle_id, taxlot_state) {
            taxlot_detail_controller_scope.item_state = taxlot_state;
            return $q.when(
              {
                status: 'success'
              }
  ...
  function create_taxlot_detail_controller() {
    ...
    taxlot_detail_controller = controller('taxlot_detail_controller', {
      $controller: controller,
      $scope: taxlot_detail_controller_scope,
  ...
  it('should have a TaxLot payload with correct object properties', function () {
    ...
    // act
    taxlot_detail_controller_scope.$digest();

    // assertions
    expect(taxlot_detail_controller_scope.taxlot.id).toBe(4);
    expect(taxlot_detail_controller_scope.cycle.id).toBe(1);
    expect(taxlot_detail_controller_scope.item_state.address_line_1).toBe("123 Main St.");
  ...
  it('should make a copy of TaxLot state while making edits', function () {
    ...
    // act
    taxlot_detail_controller_scope.$digest();
    taxlot_detail_controller_scope.on_edit();
    taxlot_detail_controller_scope.item_state.address_line_1 = "ABC Main St.";

    // assertions
    expect(taxlot_detail_controller_scope.item_copy.address_line_1).toBe("123 Main St.");
  ...
  it('should restore enabled the edit fields if a user clicks edit', function () {
    ...
    // assertions
    expect(taxlot_detail_controller_scope.edit_form_showing).toBe(true);
  ...
  it('should restore the copy of TaxLot state if a user clicks cancel', function () {
    ...
    taxlot_detail_controller_scope.on_edit();
    taxlot_detail_controller_scope.item_state.address_line_1 = "ABC Main St.";
    taxlot_detail_controller_scope.on_cancel();

    // assertions
    expect(taxlot_detail_controller_scope.item_state.address_line_1).toBe("123 Main St.");
    expect(taxlot_detail_controller_scope.edit_form_showing).toBe(false);
  ...
  it('should save the TaxLot state when a user clicks the save button', function () {
    ...
    taxlot_detail_controller_scope.on_edit();
    taxlot_detail_controller_scope.item_state.address_line_1 = "ABC Main St.";
    taxlot_detail_controller_scope.on_save();

    // assertions
    expect(mock_inventory_service.update_taxlot)
      .toHaveBeenCalledWith(taxlot_detail_controller_scope.taxlot.id,
        taxlot_detail_controller_scope.cycle.id,
        taxlot_detail_controller_scope.item_state);
    expect(taxlot_detail_controller_scope.item_state.address_line_1).toEqual("ABC Main St.");
  });
  ...
  it('should hide certain TaxLot properties, including ids and extra_data', function () {
    ...
    // assertions
    expect(taxlot_detail_controller_scope.is_valid_data_column_key('id')).toEqual(false);
    expect(taxlot_detail_controller_scope.is_valid_data_column_key('pk')).toEqual(false);
    expect(taxlot_detail_controller_scope.is_valid_data_column_key('pk_source')).toEqual(false);
    expect(taxlot_detail_controller_scope.is_valid_data_column_key('extra_data ')).toEqual(false);
    expect(taxlot_detail_controller_scope.is_valid_data_column_key('address_line_1')).toEqual(true);
