 ...
 * :author
 */
xdescribe('controller: export_modal_controller', function () {
  // globals set up and used in each test scenario
  var mock_export_service, controller, modal_state;
  var ctrl_scope;
  // make the seed app available for each test
  // 'config.seed' is created in TestFilters.html
  beforeEach(function () {
    module('BE.seed');
    inject(function ($controller, $rootScope, $uibModal, $q, export_service) {
      ...
      spyOn(mock_export_service, 'export_buildings')
        .andCallFake(function () {
          ...
          return $q.resolve({status: 'success', export_id: '123456'});
        });
  ...
  function create_export_modal_controller () {
    controller = controller('export_modal_controller', {
      $scope: ctrl_scope,
      $uibModalInstance: {
        close: function () {
          modal_state = 'close';
