...
class TestMatching(DataMappingBaseTestCase):
    ...
    def test_multiple_id_matches(self):
        ...
        # promote a properties
        ps = PropertyState.objects.filter(custom_id_1='9')
        ps_test = ps.first()
        ps_test_2 = ps.last()
        for p in ps:
        ...
        matches = tasks.query_property_matches(property_states, None, '9')
        self.assertEqual(len(matches), 2)
        self.assertEqual(matches[0], ps_test)
        self.assertEqual(matches[1], ps_test_2)
        # should return only the second property
        matches = tasks.query_property_matches(property_states, '1311528', None)
        self.assertEqual(len(matches), 1)
        ...
        matches = tasks.query_property_matches(property_states, '1311527', '9')
        self.assertEqual(len(matches), 2)
        self.assertEqual(matches[0], ps_test)
        # if passing in the second pm then it will not be the first
        matches = tasks.query_property_matches(property_states, '1311528', '9')
        self.assertEqual(len(matches), 2)
        self.assertEqual(matches[1], ps_test_2)
        ...
        matches = tasks.query_property_matches(property_states, None, '1311528')
        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0], ps_test_2)
        matches = tasks.query_property_matches(property_states, '9', None)
        self.assertEqual(len(matches), 2)
        self.assertEqual(matches[0], ps_test)
        self.assertEqual(matches[1], ps_test_2)
