...
def fix_source_type(apps, schema_editor):
    ...
    import_files.filter(source_program='PortfolioManager').update(source_type=SEED_DATA_SOURCES[PORTFOLIO_RAW][1])

    # Rename `SensorMetaData` to `SensorMetadata` for consistency
    import_files.filter(source_type='SensorMetaData').update(source_type=SEED_DATA_SOURCES[SENSOR_METADATA][1])
