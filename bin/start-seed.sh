...
if [ ! -f $CELERY_PIDFILE ]; then
    ...
    celery -A seed worker -l info -c 4 --max-tasks-per-child 1000 --events -B --scheduler django_celery_beat.schedulers:DatabaseScheduler --pidfile=$CELERY_PIDFILE > /tmp/celeryd.log 2>&1 &
else
